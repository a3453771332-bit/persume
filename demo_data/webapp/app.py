...
def run():
    ...
    with assessment:
        ...
        if s["next_unit"]:
            ...
            with gain:
                ...
                st.caption(f"Illustrative acquisition cost: {detail['illustrative_cost']:.0f} units; delay assumption: {detail['delay']}.")
        ...
        st.plotly_chart(risk_evolution_chart([{ "Step":x["Step"],"PD":x["PD after"]} for x in st.session_state.audit],e.decision_threshold),use_container_width=True)
    with policy_tab:
        ...
        if "policy_results" in st.session_state:
            ...
            chart.update_yaxes(tickformat=".1%"); chart.update_layout(height=370,margin=dict(l=10,r=10,t=35,b=10)); st.plotly_chart(chart,use_container_width=True)
    ...
    with operational_tab:
        ...
        if "operational_metrics" in st.session_state:
            ...
            st.plotly_chart(px.funnel(funnel,x="Applications",y="Stage",template="plotly_white"),use_container_width=True)
            kpi=pd.DataFrame([m]).T.rename(columns={0:"Value"}); st.dataframe(kpi,use_container_width=True)
            ...
            c1.update_yaxes(tickformat=".1%"); st.plotly_chart(c1,use_container_width=True)
            ...
            c2.update_yaxes(tickformat=".1%"); st.plotly_chart(c2,use_container_width=True)
            ...
            st.dataframe(reviews,use_container_width=True,hide_index=True,column_config={"Current PD":st.column_config.NumberColumn(format="%.1%%"),"Decision threshold":st.column_config.NumberColumn(format="%.1%%"),"Boundary distance":st.column_config.NumberColumn(format="%.1%%")})
            if len(reviews):
    ...
    with failure_tab:
        ...
        if "failures" in st.session_state:
            ...
            table=failures.loc[failures["Failure type"].isin(kind)].drop(columns=["Path","Final mask"]); st.dataframe(table,use_container_width=True,hide_index=True,column_config={"Current PD":st.column_config.NumberColumn(format="%.1%%"),"Full Evidence PD":st.column_config.NumberColumn(format="%.1%%")})
            if len(table):
                chosen=st.selectbox("Inspect failure case",table["Case ID"].tolist()); st.subheader("Failure timeline")
                st.dataframe(failure_timeline(e,chosen,st.session_state.margin),use_container_width=True,hide_index=True,column_config={"PD":st.column_config.NumberColumn(format="%.1%%"),"Boundary distance":st.column_config.NumberColumn(format="%.1%%")})
                ...
                st.dataframe(failure_policy_comparison(e,chosen,st.session_state.margin),use_container_width=True,hide_index=True)
...
- Manual Review is a product-layer queue state and never changes the frozen binary model prediction.""")
    with audit:
        st.dataframe(pd.DataFrame(st.session_state.audit),use_container_width=True,hide_index=True,column_config={"PD before":st.column_config.NumberColumn(format="%.1%%"),"PD after":st.column_config.NumberColumn(format="%.1%%")})
    with evaluation:
